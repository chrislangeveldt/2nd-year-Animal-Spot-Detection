...
def compare_pictures(file1, file2, is_greyscale=False):
    img1 = None
    img2 = None
    ...
    try:
        ...
        print("Unable to open image {}: {}".format(file2, e))
        return 0
    ...
    for i in range(img1.width):
        for j in range(img1.height):
            ...
            if  abs(c1[0] - c2[0]) <= threshold and \
...
def run_tests(bin_path, tests, outs, mode, char_threshold=200):
    ...
    for test, out_case in zip(tests, outs):
        ...
        out = compare_pictures(expected, received, is_greyscale=(mode=="gs"))
